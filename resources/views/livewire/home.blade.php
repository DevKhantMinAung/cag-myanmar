<div x-data="dropdown">
    ...
    {{-- Hero Section --}}
    <section class="h-[50vh] min-[1025px]:h-screen">
    ...
    {{-- About Section --}}
    <section id="about" class="w-screen cag_container">
    ...
    @if ($excutive_profile)
        <div id="profile" class="section text-center">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-8 xl:px-20 mt-8 lg:mt-16">
                <div class="w-full flex justify-center xl:justify-end items-center">
                ...
                <div class="text-start space-y-4 sm:space-y-6 flex flex-col justify-center p-4">
                    <h3 class="text-xl sm:text-2xl xl:text-4xl text-slate-900 font-bold">{{ $excutive_profile->name }}
                    </h3>
                    <div>
                        <h5 class="text-slate-600 text-sm sm:text-lg xl:text-xl font-bold">
                            {{ $excutive_profile->title1 }}</h5>
                        <h5 class="text-slate-600 text-sm sm:text-lg xl:text-xl font-bold">
                            {{ $excutive_profile->title2 }}</h5>
                    </div>
                    <p class="text-xs/5 sm:text-base/8 text-justify sm:text-start text-slate-800">
                        {{ $excutive_profile->desc }}</p>
                </div>
            </div>
        </div>
    @endif
    ...
    {{-- Key Sector Section --}}
    <section id="key-sector" class="section">
        <h1 class="section-title text-center">Diverse Expertise Across <span>Key Sectors</span></h1>
        <p class="section-intro text-center">CAG Group of Companies brings together specialized knowledge and experience
            from various industries, offering comprehensive solutions in Solar Engineering, Media Production, Medical
            Devices, Wastewater Treatment, Marketing, and Event Management.</p>

        <div>
            <div class="swiper key-swiper relative pt-16">
                <!-- Additional required wrapper -->
                <div class="swiper-wrapper">
                    <!-- Slides -->

                    @foreach ($key_sectors as $item)
                        <div
                            class="swiper-slide bg-white px-4 py-8 rounded-lg shadow-lg border border-slate-200 h-full">
                            <div class="">
                                <div
                                    class="aspect-video mx-auto border border-slate-200 rounded-lg overflow-hidden mb-6">
                                    <img class="w-full h-full bg-center bg-cover"
                                        src="{{ '/storage/' . $item->image }}" alt="thumbnail">
                                </div>
                                <h3 class="text-start text-xl font-semibold text-cag_green py-4">{{ $item->title }}
                                </h3>
                                <p class="text-sm/6 text-slate-800">{{ $item->intro }}</p>
                ...
                <div class="absolute top-6 left-0 w-full flex justify-between items-center px-4 bg-red-500 !important">
    ...
    @if ($records)
        <section id="records" class="section">
            ...
            <h1 class="section-title text-center">Proven <span>Track Records</span></h1>
            <p class="section-intro text-center">CAG Group of Companies brings together specialized knowledge and
                experience
                from various industries, offering comprehensive solutions in Solar Engineering, Media Production,
                Medical
                Devices, Wastewater Treatment, Marketing, and Event Management.</p>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach ($records as $record)
                    <div
                        class="w-full h-full bg-white p-4 rounded-lg shadow-lg border border-slate-200 flex flex-col space-y-6">
                        <div class="aspect-video">
                            <img class="w-full h-full bg-cover bg-center" src="{{ '/storage/' . $record->thumbnail }}"
                                alt="record-image">
                        </div>
                        <h3 class="text-lg font-semibold text-black line-clamp-2 text-wrap flex-grow">
                            {{ $record->title }}</h3>
                        <p class="text-sm/6 text-slate-700 line-clamp-3 text-wrap ">{{ $record->intro }}</p>

                        <div>
                            <button
                                class="inline px-4 py-2 text-white bg-cag_green rounded hover:scale-105 transition-all duration-150 ease-linear"
                                @click='openRecordModal({
                                    name: "{{ $record->title }}",
                                    intro: "{{ $record->intro }}",
                                    thumbnail: "{{ $record->thumbnail }}",
                                    images: {{ json_encode($record->images()) }}
                                })'>
                                View Record
                            </button>
                        </div>
    ...
    <Footer
        class=" bg-white pt-4 sm:pt-6 md:pt-8 shadow-[0px_4px_16px_rgba(17,17,26,0.1),_0px_8px_24px_rgba(17,17,26,0.1),_0px_16px_56px_rgba(17,17,26,0.1)] border border-slate-200 relative">
        <div class="cag_container py-4 sm:py-6 md:py-8">
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-x-8 py-4">
                <div class="space-y-6 col-span-2 border-b sm:border-0 border-slate-200 py-6">
                    ...
                    <ul class="space-y-4">
                        <li class="flex gap-6">
                            ...
                            <div>
                                <a href="">Building (2), SH-B6, Malikha Housing, Yadanar Road, Thingangyun
                                    Township, Yangon, Myanmar</a>
                            </div>
                        </li>
                        <li class="flex gap-6">
                            <div class="max-w-12 text-cag_green">
                ...
                <div class="space-y-6 py-6">
                    ...
                    <ul class="space-y-2">
                        <li><a href="#" class="text-slate-800 hover:text-cag_green transition-colors">Home</a>
                        </li>
                        <li><a href="#about" class="text-slate-800 hover:text-cag_green transition-colors">About</a>
                        </li>
                        <li><a href="#companies"
                                class="text-slate-800 hover:text-cag_green transition-colors">Companies</a></li>
                        <li><a href="#services"
                                class="text-slate-800 hover:text-cag_green transition-colors">Services</a></li>
                        <li><a href="#records"
                                class="text-slate-800 hover:text-cag_green transition-colors">Records</a></li>
                    </ul>
                </div>
                ...
                <div class="space-y-6 py-6">
                    ...
                    <ul class="flex gap-4">
                        <li><a href="#" class="text-slate-800 hover:text-cag_green transition-colors">
                                <svg class="w-8 sm:w-10" fill="#1b25b6" viewBox="-5.5 0 32 32" version="1.1"
                        ...
                        <li><a href="#" class="text-slate-800 hover:text-cag_green transition-colors">
                                <svg class="w-8 sm:w-10" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg"
                                    fill="none">
                                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                    <g id="SVGRepo_iconCarrier">
                        ...
                        <li><a href="#" class="text-slate-800 hover:text-cag_green transition-colors">
                                <svg class="w-8 sm:w-10" viewBox="0 0 32 32" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                    <g id="SVGRepo_iconCarrier">
                        ...
                        <li><a href="#" class="text-slate-800 hover:text-cag_green transition-colors"><i
    ...
    {{-- Record Popup Modal --}}
    <div class="bg-white/80 fixed w-full h-full left-0 top-0 flex items-center justify-center z-20"
        x-show="recordModal" x-transition:enter="transition ease-out duration-300"
        x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100"
        x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 scale-100"
        x-transition:leave-end="opacity-0 scale-90">
        <div
            class="bg-white relative w-full max-w-[90%] md:max-w-[750px] mx-auto p-3 pt-8 md:p-6 rounded shadow-lg border-2 border-slate-200 text-start z-50 space-y-6 flex flex-col">
            <button type="button"
                class=" absolute top-3 right-3 p-1 rounded-full shadow border border-slate-200 text-slate-700 hover:scale-110 transition-transform ease-linear"
            ...
            <div class="swiper record-swiper relative w-full sm:aspect-[16/9] sm:max-w-[600px] overflow-hidden">
                <div class="absolute w-full h-full top-0 left-0 flex justify-between items-center">
                    <button class="swiper-button-prev"></button>
                    <button class="swiper-button-next"></button>
                </div>
                <div class="swiper-wrapper">
                    <div class="swiper-slide mx-auto overflow-hidden border-4 border-cag_green w-full">
                        <img class="bg-cover bg-center"
                            src="https://images.pexels.com/photos/1037992/pexels-photo-1037992.jpeg?auto=compress&cs=tinysrgb&w=1280"
                            alt="">
                    </div>
                    <div class="swiper-slide mx-auto overflow-hidden border-4 border-cag_green w-full">
                        <img class="bg-cover bg-center"
                            src="https://images.pexels.com/photos/1054218/pexels-photo-1054218.jpeg?auto=compress&cs=tinysrgb&w=1280"
                            alt="">
                    </div>
                    <div class="swiper-slide mx-auto overflow-hidden border-4 border-cag_green w-full">
                        <img class="bg-cover bg-center"
                            src="https://images.pexels.com/photos/776656/pexels-photo-776656.jpeg?auto=compress&cs=tinysrgb&w=1280"
                            alt="">
                    </div>
                </div>
                <div class="swiper-pagination"></div>
            ...
            <h3 class="text-xl font-semibold mb-4 text-center">Record 1</h3>
            <p class="text-sm/6 md:text-base/7 text-gray-800 mb-8 px-8">Lorem ipsum dolor sit amet consectetur,
                adipisicing
                elit. Omnis eveniet atque, soluta dolor provident nemo animi repellendus. Minima, quis ex.</p>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('dropdown', () => ({
            ...
            openRecordModal(currentRecord) {
                this.currentRecord = currentRecord;
                this.recordModal = true;
                console.log("Images:", this.currentRecord.images[0]);
            },
            closeRecordModal(currentRecord) {
